...
Скрипты подключают Android `1.0.10` к приложению, проверяют Gradle-плагин и сквозной сценарий, измеряют производительность и маршрутизируют проверки монорепозитория.

Актуальный CLI имеет версию `1.0.0`, создаёт JHLOG `5.1.0` и сохраняет чтение `5.0.0`. При обновлении Android-компонентов обновляйте и сам исполняемый файл CLI; прежняя сборка с тем же номером может не поддерживать новые обязательные возможности формата.
...
Сначала выполните [подготовку Gradle-плагина](#подготовка-gradle-плагина) в Maven-каталоге целевого приложения. Интегратор вызывает корневую публикацию SDK, а composite Gradle-плагин публикуется отдельно.
...
Скрипт читает `jankHunterVersion=1.0.10` из `android/gradle.properties` текущей рабочей копии и использует эту версию для плагина и `jankhunter-android-sdk`.

### Подготовка Gradle-плагина

Для стандартного каталога `<target>/.jankhunter/maven` выполните из корня Jank Hunter:
...
Замените путь на фактический Maven-каталог приложения; при `--maven-dir` укажите соответствующий абсолютный путь. Публикация в обычный `~/.m2/repository` не обновляет этот каталог. Повторяйте отдельную публикацию при обновлении исходников плагина, даже если номер остаётся `1.0.10`.

Для обычного Maven Local без интегратора нужны обе команды из каталога `android`:
...
После перезаписи той же версии выполните Gradle sync или сборку приложения с `--refresh-dependencies`. Параметры `--skip-publish` и `--skip-cli-build` предполагают, что актуальные артефакты и CLI уже подготовлены; они не обновляют их автоматически.
...
5. Добавляет плагин `io.jankhunter.android` версии `1.0.10`.
6. Добавляет единую зависимость `io.jankhunter:jankhunter-android-sdk:1.0.10`.
...
`--verify` запускает `<module>:tasks` и проверяет разрешение Gradle-конфигурации. Для проверки сборки после интеграции отдельно выполните в приложении `./gradlew :app:assembleDebug`, подставив свой модуль.
...
1. Публикует плагин и Android-артефакты `1.0.10` в изолированный Maven-репозиторий.
...
При ручной проверке JSON учитывайте `diagnostic_completeness_percent=-1` как неизвестность, а не процент. Legacy TTFB, неизвестные HTTP-интервалы и HPROF-сопоставление только по классу не являются полными измерениями. В HTML подробности качества находятся в «Математический анализ → Качество данных», а не среди проблем приложения.
...
`android_runtime` в этом скрипте означает host-JVM unit benchmarks, а не запуск на ART. Для изменений горячих путей отдельно нужны измерения на устройстве или эмуляторе с одинаковым workload и проверкой записанных событий. Не смешивайте JVM, ART, RSS процесса и объём суммарных аллокаций в один показатель.
...
./scripts/sync-github-master-to-gitlab.sh --version 1.0.10 --branch <ветка>
```
