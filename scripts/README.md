...
Скрипты подключают Android `1.0.11` к приложению, проверяют Gradle-плагин и сквозной сценарий, измеряют производительность и маршрутизируют проверки монорепозитория.
...
Скрипт читает `jankHunterVersion=1.0.11` из `android/gradle.properties` текущей рабочей копии и использует эту версию для плагина и `jankhunter-android-sdk`.
...
Замените путь на фактический Maven-каталог приложения; при `--maven-dir` укажите соответствующий абсолютный путь. Публикация в обычный `~/.m2/repository` не обновляет этот каталог. Повторяйте отдельную публикацию при обновлении исходников плагина, даже если номер остаётся `1.0.11`.
...
5. Добавляет плагин `io.jankhunter.android` версии `1.0.11`.
6. Добавляет единую зависимость `io.jankhunter:jankhunter-android-sdk:1.0.11`.
...
1. Публикует плагин и Android-артефакты `1.0.11` в изолированный Maven-репозиторий.
...
./scripts/sync-github-master-to-gitlab.sh --version 1.0.11 --branch <ветка>
```
