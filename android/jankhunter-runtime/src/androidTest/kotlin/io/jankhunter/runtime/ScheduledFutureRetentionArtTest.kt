...
class ScheduledFutureRetentionArtTest {
    @Test
    fun ordinaryExecutorKeepsTheDelegatesRetrySemantics() {
        val delegate = Executor { command ->
            try {
                command.run()
        ...
        var attempts = 0
        JankHunterExecutor(delegate, "retry", null, { 1L }, activeExecutorTestCallbacks()).execute {
            attempts++
