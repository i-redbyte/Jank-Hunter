...
- `jankhunter-android-sdk`: единый SDK-артефакт, который транзитивно подключает runtime, annotations и OkHttp helpers.
...
Плагин сам добавит `compileOnly("io.jankhunter:jankhunter-annotations:1.0.0")` и `debugImplementation("io.jankhunter:jankhunter-android-sdk:1.0.0")`. Если в `jankHunter.enabledBuildTypes` добавить, например, `qa`, SDK будет добавлен и в `qaImplementation`.

Если Gradle-плагин временно нельзя использовать, подключите единый SDK вручную:
...
- подключает `io.jankhunter.android`; Maven-зависимости SDK добавляются самим Gradle-плагином;
...
Для application-модуля ASM по умолчанию остаётся в `InstrumentationScope.ALL` даже в release-like вариантах, чтобы перехваты вроде OkHttp работали и в коде зависимостей. Ограничивайте область через `includePackages`/`excludePackages`, если нужно сузить покрытие.
