...
- Core runtime написан на Java.
- Core runtime не зависит от Kotlin stdlib, AndroidX, OkHttp, RxJava или Compose.
...
```java
JankHunterConfig config = JankHunterConfig.builder()
        .enabled(true)
...
```java
JankHunter.withOwner("FeedRepository.refresh", new Runnable() {
    @Override
    public void run() {
...
OkHttpClient client = new OkHttpClient.Builder()
        .eventListenerFactory(new JankHunterEventListenerFactory())
        .build();
```
...
```java
EventListener.Factory existing = ...;

OkHttpClient client = new OkHttpClient.Builder()
        .eventListenerFactory(new JankHunterEventListenerFactory(existing))
        .build();
```
