...
Одна сессия сбора пишет ровно один файл. Локальная дата фиксируется при старте сессии, а десятичный `index` монотонно растёт и не переиспользуется. Имя файла намеренно не содержит имени процесса: идентификаторы запуска, процесса и сессии хранятся в заголовке, и утилита группирует данные по содержимому, а не по имени. Уход приложения в фон только сбрасывает уже накопленные блоки и не начинает новый файл.
...
хранилище пишет кольцевой `.jhlog 1.0`: после заполнения вытесняет старейшие целые блоки и
продолжает ту же сессию в том же файле. Физический размер не превышает выбранный предел.
...
используется меньшее значение. Пользовательское хранилище без произвольного доступа продолжает
получать последовательный формат 9; хранилище с `JankHunterRandomAccessBinaryStorage` получает
формат 1.0.
...
`.jhlog 1.0` хранит независимо сжатые блоки с контрольными суммами и отметками завершённой
записи. Два чередующихся служебных заголовка позволяют восстановить последнее целое состояние
после прерывания записи. Утилита читает блоки в логическом порядке кольца и игнорирует
незавершённую запись. Чтение прежних форматов 8 и 9 временно сохранено отдельными обработчиками.
...
История ограничена последними 1024 сессиями и 400 днями. В каждый новый `.jhlog 1.0`
