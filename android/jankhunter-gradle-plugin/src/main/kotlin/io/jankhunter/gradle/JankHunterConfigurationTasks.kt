...
internal object JankHunterConfigurationValidation {
    ...
    fun failures(
        ...
        if (VariantBuildTypeMatcher.isReleaseLike(configuration.variant.buildType)) {
            validateRelease(configuration, failures, artTiPackaged)
        }
        return failures
    }
    ...
    private fun validateRelease(
        ...
        if (artTiPackaged && !safety.allowArtTiAgent) {
            failures += "ART TI in release requires allowArtTiAgent()."
        }
    }
