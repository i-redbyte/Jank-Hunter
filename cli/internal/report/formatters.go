...
func significantMathFindings(findings []mathanalysis.Finding) []mathanalysis.Finding {
	...
	for _, finding := range findings {
		if isSignificantSeverity(finding.Severity) && !isInternalCollectionLimitation(finding.Detail) {
...
func integralHelp(id string) string {
	...
	case "network_failure_burn":
		return "Условная нагрузка объединяет HTTP-ошибки, всплески DNS/соединений и повторяемые циклы. Это не время, трафик или расход батареи. Сравнивайте число только для одинаковых сценариев."
...
func scoreHelp(kind string) string {
	...
	case "network_burn":
		return "Условная нагрузка сетевого цикла зависит от числа и размера всплесков и надёжности сигнала. Это не миллисекунды, байты или расход батареи. До 5 - слабый сигнал, 5-20 - проверить, выше 20 - высокий приоритет."
	case "confidence":
		return "Надёжность лежит в диапазоне от 0 до 1. Чем ближе к 1, тем лучше сигнал подтверждён повторами, количеством данных или несколькими методами."
	case "path_cost":
		return "Условная стоимость цепочки связей: меньше означает более короткую цепочку с надёжными связями. Это не вероятность и не доказательство причины."
	case "integral":
	...
	default:
		return "Оценка - относительный приоритет внутри этого отчёта. Смотрите рядом критерии, надёжность, количество данных и контекст."
	}
}
...
func scoreGuideHTML(kind string) template.HTML {
	...
	case "leak":
		return template.HTML(`<div class="score-guide"><div class="score-guide-card"><strong>Шкала сигналов удержания</strong><span class="score-band sev-ok">до 7: наблюдать</span><span class="score-band sev-medium">7-16: проверить</span><span class="score-band sev-high">16+: высокий приоритет</span><p>Наблюдение только после задержки получает меньший вес, после явного GC - средний, а подтверждённый путь HPROF - полный. Оценка задаёт порядок проверки и сама по себе не доказывает утечку.</p></div></div>`)
	case "math":
		return template.HTML(`<div class="score-guide"><div class="score-guide-card"><strong>Шкала математических оценок</strong><span class="score-band sev-ok">0-3: слабый сигнал</span><span class="score-band sev-medium">3-6: проверить</span><span class="score-band sev-high">6+: высокий приоритет</span><p>Шкала задаёт порядок проверки внутри похожих сценариев. Надёжность показывает поддержку данными, а не вероятность ошибки в коде.</p></div></div>`)
	case "compare":
	...
	default:
		return template.HTML(`<div class="score-guide"><div class="score-guide-card"><strong>Как читать оценку</strong><p>Оценка - это относительный приоритет внутри этого отчёта. Смотрите рядом критерии, надёжность, количество данных и контекст.</p></div></div>`)
	}
}

func httpCountValue(bucket mathanalysis.TimelineBucket, count int) string {
	if bucket.HTTPCountState == "" || bucket.HTTPCountState == mathanalysis.CountObserved {
