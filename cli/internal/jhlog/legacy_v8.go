...
func streamBinaryV8(file io.Reader, result StreamResult, handle EventHandler) (StreamResult, error) {
	body, closeBody, err := gzipBinaryBody(file)
	if err != nil {
		return corruptResult(result, fmt.Errorf("compressed v8 body: %w", err))
	}
	if closeBody != nil {
		defer closeBody.Close()
	}
	wrapped := func(event Event, dict map[uint64]string) error {
		result.TotalRecords++
		...
		return handle(event, dict)
	}
	log, err := streamBinaryV8Body(body, result.Source, LegacyFormatVersion8, wrapped)
	result.Warnings = append(result.Warnings, log.Warnings...)
	if err != nil {
		return result, err
	}
	result.Version = LegacyFormatVersion8
	result.FormatMajor = 0
	result.FormatMinor = 0
...
type compactDecodeState struct {
	lastContext legacyV8ContextTuple
	hasContext  bool
}

type legacyV8ContextTuple struct {
	screenID uint64
	ownerID  uint64
	flowID   uint64
	stepID   uint64
...
func decodePayload(payload []byte, event *Event, decodeState *compactDecodeState) error {
	...
	case EventContext:
		...
		context := &ContextEvent{
			...
			BatteryTempDeciC: decodeV8SVarint(values[4]),
			RxBytes:          values[5],
			TxBytes:          values[6],
...
func readContextIDs(read func() (uint64, error), flags uint64, decodeState *compactDecodeState) (screenID, ownerID, flowID, stepID uint64, err error) {
	...
	if decodeState != nil {
		decodeState.lastContext = legacyV8ContextTuple{screenID: screenID, ownerID: ownerID, flowID: flowID, stepID: stepID}
...
func metricFromValues(values []uint64, eventType EventType) *MetricEvent {
	event := &MetricEvent{
		...
		Sum:      values[1],
		Max:      values[1],
		Mode:     legacyV8DefaultMetricMode(eventType),
	}
	if len(values) > 2 {
...
func legacyV8DefaultMetricMode(eventType EventType) MetricMode {
...
func decodeV8SVarint(value uint64) int64 {
