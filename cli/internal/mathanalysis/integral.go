...
func computeIntegralScores(timeline []TimelineBucket, loops []NetworkLoopFinding) []IntegralScore {
	definitions := []integralDefinition{
		...
		},
		{
			...
			explanation: "Интегрирует только хвост задержки выше инженерного целевого порога 300 ms.",
			unit:        "ms*с",
		...
		},
		{
			...
			formula:     "Σ (HTTP_ошибки + 0.25*DNS + 0.25*connect) * Δt + Σ выгорание_цикла",
			explanation: "Объединяет ошибки, DNS/connect всплески и оценку выгорания найденных сетевых циклов.",
			unit:        "усл.ед.",
