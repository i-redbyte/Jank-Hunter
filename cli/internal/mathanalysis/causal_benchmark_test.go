...
func BenchmarkJointCausalPipeline(b *testing.B) {
	for _, owners := range []int{1, 16} {
		b.Run(fmt.Sprintf("owners_%d_20k", owners), func(b *testing.B) {
			...
			for i := 0; i < 20000; i++ {
				event := jhlog.Event{Type: jhlog.EventHTTP, TimeMS: uint64(i * 2), Attribution: jhlog.AttributionContext{Present: true, Owner: jhlog.LocalSymbol(uint64((i/32)%owners + 1))}, HTTP: &jhlog.HTTPEvent{RouteRef: jhlog.LocalSymbol(uint64(owners + i%32 + 1)), DurationMS: 100, Status: jhlog.Status2xx}}
				if i%16 == 0 {
			...
			for i := 0; i < b.N; i++ {
				...
				if err != nil || report.Summary.HTTPCount != 20000 || len(report.CollectionLimits) != 0 {
