...
func (b *problemBuilder) detectMemory() {
	for _, leak := range b.summary.MemoryLeaks {
		...
		exposure := boundedUint64Score(leak.Count, 5, 3, 20)
...
func (b *problemBuilder) detectProcessExit() {
	for _, exit := range b.summary.ProcessExits {
		...
		b.add(ProblemFinding{
			...
			PriorityBreakdown: priority(40, 20, boundedUint64Score(exit.Count, 6, 3, 20), locationBreadth(where), 3, "аварийное завершение, ANR или OOM", "системная причина", "исторические события", "процесс", "причина, время и память"),
...
func (b *problemBuilder) detectCPU() {
	value, ok := namedGaugeValue(b.summary.Gauges, "process.cpu.core_percent_x100")
	if !ok {
		return
	}
...
func (b *problemBuilder) detectAsyncQueues() {
	...
	for _, executor := range b.summary.AsyncAnalysis.Executors {
		...
		b.add(ProblemFinding{
			...
			Impact:       []string{"Фоновые результаты приходят позже и задерживают зависящие от них UI- и сетевые сценарии"},
			Evidence: []ProblemEvidence{
			...
			PriorityBreakdown: priority(22, boundedUint64RatioScore(executor.MaxWaitMS, b.cfg.AsyncQueueWaitMS, 8, 4, 25), min(20, 5+int(math.Log2(float64(executor.WaitSamples)+1))*3), locationBreadth(where), compound, "задержка результата", "время ожидания в очереди", "число запусков", "пул потоков", "насыщение пула"),
			Recommendations:   []ProblemRecommendation{{Action: "Проверить размер пула, блокирующие задачи и приоритет его очереди", Rationale: "Известно, в каком пуле было ожидание, но виновная задача пока не определена.", Verification: "Повторить сценарий и подтвердить снижение максимального и среднего ожидания и глубины очереди."}},
			Limitations:       append(limits, "Метрики объединены по окнам: максимум точен, среднее взвешено по числу замеров, исходное распределение восстановить нельзя."),
...
func (b *problemBuilder) detectGCBlocking() {
	...
	b.add(ProblemFinding{
		...
		Why:          ProblemWhy{ClaimLevel: "correlated", Summary: fmt.Sprintf("Счётчики ART подтверждают нагрузку от GC. %d окон с подтормаживаниями UI совпали по времени с измерением GC, но это не доказывает причинную связь.", gc.JankyUIWindowsNearGC)},
		...
		Evidence: []ProblemEvidence{
			...
			{Name: "Подтормаживания UI рядом с GC", Observed: fmt.Sprint(gc.JankyUIWindowsNearGC), Unit: "events", Source: "temporal_window_join"},
		},
		Frequency:         &ProblemFrequency{Count: gc.BlockingCount},
		PriorityBreakdown: priority(28, boundedUint64RatioScore(gc.BlockingTimeMS, b.cfg.GCBlockingTimeMS, 8, 5, 25), boundedUint64Score(gc.BlockingCount, 4, 3, 20), 1, boolScore(gc.JankyUIWindowsNearGC > 0, 4), "паузы процесса", "время блокирующих GC", "число GC", "весь процесс", "совпавшие UI-окна"),
		...
		Limitations:       append(limits, "Время события может соответствовать концу объединённого окна; совпадение с UI по времени не доказывает причину."),
...
func (b *problemBuilder) detectStartupLatency() {
	...
	if startup.ColdResumeSamples > 0 && startup.MaxColdResumeMS >= b.cfg.StartupColdResumeMS {
		...
		b.add(ProblemFinding{
			...
			PriorityBreakdown: priority(30, boundedUint64RatioScore(startup.MaxColdResumeMS, b.cfg.StartupColdResumeMS, 10, 5, 25), boundedUint64Score(startup.ColdResumeSamples, 5, 3, 20), 1, 0, "первый экран", "длительность первого открытия", "число запусков", "весь процесс", "один сигнал жизненного цикла"),
	...
	for _, screen := range startup.Screens {
		...
		b.add(ProblemFinding{
			...
			Confidence: confidence, ConfidenceReasons: reasons,
			Title:             fmt.Sprintf("Экран %s долго переходит от создания к активному состоянию", displayUnknown(screen.Screen, "экран не определён")),
			...
			PriorityBreakdown: priority(24, boundedUint64RatioScore(screen.MaxResumeMS, b.cfg.ScreenResumeMS, 8, 5, 25), boundedUint64Score(screen.ResumeSamples, 4, 3, 20), locationBreadth(where), 0, "переход экрана", "время до активного состояния", "число переходов", "экран", "один сигнал жизненного цикла"),
			Recommendations:   []ProblemRecommendation{{Action: "Проверить синхронную работу между onCreate, onStart и onResume Activity и перенести необязательную подготовку", Rationale: "Известен класс проблемного экрана.", Verification: "Повторить переход и сравнить среднее и максимальное время до активного состояния."}},
...
func (b *problemBuilder) detectPower() {
	measurement, samples, ok := namedGaugeMeasurement(b.summary.Gauges, "device.thermal.status")
	peakStatus := maxUint64(measurement.Value, measurement.maximum)
	if !ok || peakStatus < b.cfg.ThermalSevereStatus {
		return
	}
	confidence, reasons, limits := problemConfidence(b.summary, samples, thermalMinimumSamples, true)
	b.add(ProblemFinding{DetectorID: "power.thermal_pressure", DetectorVersion: b.cfg.Version, Category: ProblemCategoryPower, Subcategory: "thermal", Status: "observed", Confidence: confidence, ConfidenceReasons: reasons, Title: "Устройство работало при сильном нагреве", WhatHappened: fmt.Sprintf("Максимальный записанный статус нагрева Android - %d (порог сильного нагрева: %d), замеров: %d.", peakStatus, b.cfg.ThermalSevereStatus, samples), Why: ProblemWhy{ClaimLevel: "unknown", Summary: "Состояние нагрева - внешний фактор; оно не доказывает, что приложение вызвало нагрев."}, Impact: []string{"Снижение частот процессора из-за нагрева может усиливать просадки CPU и интерфейса"}, Evidence: []ProblemEvidence{{Name: "Максимальный статус нагрева", Observed: fmt.Sprint(peakStatus), Unit: "Android status", ExpectedOrThreshold: fmt.Sprintf("< %d", b.cfg.ThermalSevereStatus), Sample: u64ptr(samples), Source: "system_sampler_gauge"}}, PriorityBreakdown: priority(18, boundedUint64Score(peakStatus, 0, 4, 25), 8, 2, 0, "ограничение частот", "максимальный статус нагрева", "число замеров", "уровень устройства", "причина не связана"), Recommendations: []ProblemRecommendation{{Action: "Повторить операцию на холодном устройстве и сопоставить загрузку процессора и плавность интерфейса", Rationale: "Так отделяется дефект приложения от внешнего влияния нагрева.", Verification: "Сравнить одинаковую операцию при нормальном и сильном нагреве."}}, Limitations: append(limits, "Статус нагрева не доказывает длительность или источник нагрева.")})
}
