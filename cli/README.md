...
Текущая версия утилиты - `1.0.0`; она создаёт JHLOG `5.1.0` и читает `5.1.0` и `5.0.0`. Для Android SDK и Gradle-плагина используется `1.0.11`. CLI нужно обновлять вместе с ними: одинаковый номер `1.0.0` у старой и новой сборки не означает одинаковую поддержку формата.
