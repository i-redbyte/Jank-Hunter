...
- отдельная страница «Анализ JVM TI», если агент был включён: практические выводы о совпадении
  задержек со сборкой мусора, ожиданием блокировок и наблюдаемыми методами; служебные данные агента
  скрыты в сворачиваемой технической части;
...
Каждый вывод явно разделяет уровень доказательности, измерения, подтверждённые и недостающие данные,
ограничения качества, альтернативные объяснения, действия и ссылку на временное окно. Значение
`DIRECT` означает прямое измерение интервала, снимка стека или пересечения, а не автоматически
...
В JSON сравнения поле `Agent` сначала сообщает о различиях настроек, возможностей и качества данных,
затем перечисляет новые или исчезнувшие наблюдаемые методы и причинные связи. Показатели JVM TI
считаются несопоставимыми, если в одной стороне нет событий агента; отпечатки стеков и идентификаторы
потоков между процессами не сравниваются.
