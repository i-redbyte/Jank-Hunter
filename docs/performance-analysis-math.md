...
- каждый timeline bucket классифицируется в `Healthy`, `NetworkLoop`, `NetworkSlow`, `Janky`,
  `Stalled`, `MemoryPressure` или `Recovering`;
- `NetworkLoop` берется из найденных сетевых циклов, `Stalled` из main-thread stall buckets,
  `MemoryPressure` из роста PSS/низкой свободной RAM;
- строится матрица переходов с вероятностями;
- считаются Healthy -> bad transitions, вероятность bad -> Healthy/Recovering, ожидаемое число окон до восстановления,
  а также липкие состояния с self-transition probability;
- compare показывает дельты по переходам, восстановлению и липкости плохих состояний.
...
### Stage 8: Markov State Model

Добавлено поверх timeline/network/integral foundation:

- state sequence по каждому bucket;
- transition matrix;
- Healthy -> bad transitions;
- bad -> Healthy/Recovering recovery probability;
